...
var color;
(function (color) {
    color.phi = 1.618033988749895;
    color.HslHMin = -Math.PI;
    color.HslHMAx = Math.PI;
    color.HslSMin = 0.0;
    color.HslSMAx = 2.0 / 3.0;
    color.HslLMin = 0.0;
    color.HslLMAx = 1.0;
    color.rLumaRate = 0.299;
    color.gLumaRate = 0.587;
    color.bLumaRate = 0.114;
    color.rgbForStyle = function (expression) {
    ...
    color.rgbFromStyle = function (style) {
        var r = 0.0;
        var g = 0.0;
        var b = 0.0;
    ...
    color.xyzToLength = function (xyz) { return Math.sqrt(Math.pow(xyz.x, 2) + Math.pow(xyz.y, 2) + Math.pow(xyz.z, 2)); };
    color.rgbToXyz = function (expression) { return ({ x: expression.r, y: expression.g, z: expression.b }); };
    color.rgbToHue = function (expression) {
        var hueXy = {
    ...
    color.rgbToLuma = function (expression) { return (expression.r * color.rLumaRate) + (expression.g * color.gLumaRate) + (expression.b * color.bLumaRate); };
    color.rgbToLightness = function (expression) { return (expression.r + expression.g + expression.b) / 3.0; };
    color.calcSaturation = function (expression) {
        var lightness = color.rgbToLightness(expression);
        return color.xyzToLength({ x: expression.r - lightness, y: expression.g - lightness, z: expression.b - lightness });
    };
    color.rgbToSaturation = function (expression) { return color.calcSaturation(expression) * color.calcSaturation({ r: 1.0, g: 0.0, b: 0.0 }); };
    color.rgbToHsl = function (expression) {
        return ({
            h: color.rgbToHue(expression),
            s: color.rgbToSaturation(expression),
            l: color.rgbToLightness(expression)
        });
    };
    color.hslToRgbElement = function (expression, Angle) { return expression.l + expression.s * Math.cos(expression.h - (Math.PI * 2) / 3.0 * Angle); };
    color.hslToRgb = function (expression) {
        return ({
            r: color.hslToRgbElement(expression, 0.0),
            g: color.hslToRgbElement(expression, 1.0),
            b: color.hslToRgbElement(expression, 2.0)
        });
    };
    color.regulateHue = function (expression) {
        var h = expression.h;
        while (h < -Math.PI) {
            h += Math.PI * 2;
        }
    ...
    color.clipLightness = function (expression) {
        return ({
            h: expression.h,
            s: expression.s,
    ...
    color.clipSaturation = function (expression) {
        var rgb = color.hslToRgb(expression);
    ...
    color.regulateHsl = function (expression) { return color.clipSaturation(color.clipLightness(color.regulateHue(expression))); };
    color.clipRgb = function (expression) {
        return ({
